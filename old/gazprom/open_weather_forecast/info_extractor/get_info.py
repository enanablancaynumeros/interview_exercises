...
class GetInfo(GetInfoAbstract):
    ...
    def filter_information(self, info_retrieved, schema, result=None):
        ...
        for key in schema:
            if isinstance(info_retrieved, dict):
                ...
                for element, sub_info in zip(
                    info_retrieved, schema * len(info_retrieved)
                ):
            ...
            info_type = (
                type(schema.get(key))
                if isinstance(schema.get(key), (list, dict))
                else schema.get(key)
            )
            if not self.equivalent_types(
                value, schema.get(key)
            ) and info_type is not type(value):
                import ipdb
            ...
            if isinstance(value, (list, dict)):
                ...
                self.filter_information(
                    info_retrieved.get(key), schema.get(key), result.get(key)
                )
            else:
        ...
        if not temperatures_history:
            ...
            try:
                if information_schema:
                    temperatures_history = self.filter_information(
                        temperatures_history, information_schema
                    )
    ...
    def download_store_new_data(self, url="", information_schema=None):
        error, info_filtered_by_schema = self.get_info(
            url=url, information_schema=information_schema
        )
        if not error:
