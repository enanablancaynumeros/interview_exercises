...
def t2_subtree_t1(given_tree, subtree):
    ...
    nodes_like_subtree_root = find_nodes_equal_to(
        tree=given_tree, value=subtree.root_node.value
    )
...
def is_subtree_from_node(bigger_tree_current_node, subtree_current_node):
    if bigger_tree_current_node.value == subtree_current_node.value:
        if (
            bigger_tree_current_node.left is not None
            and subtree_current_node.left is not None
        ):
        ...
        if (
            bigger_tree_current_node.right is not None
            and subtree_current_node.right is not None
        ):
